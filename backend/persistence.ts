...
export async function persistState(state: SystemExecutionState): Promise<void> {
  ...
  const severity = state.zones.some((zone) => zone.severity === 'CRITICAL')
    ? 'CRITICAL'
    : state.zones.some((zone) => zone.severity === 'HIGH')
      ? 'HIGH'
      : 'MODERATE';

  const now = new Date();

  await prisma.disasterIncident.upsert({
    where: { id: incidentId },
    create: {
      id: incidentId,
      eventType: state.scenarioTitle,
      status: state.isRunning ? 'RUNNING' : 'ACTIVE',
      severity,
    },
    update: {
      eventType: state.scenarioTitle,
  ...
  for (const zone of state.zones as any[]) {
    await prisma.zone.upsert({
      ...
      create: {
        ...
        rainfallIntensity: Number.isFinite(zone.rainfallIntensity)
          ? Number(zone.rainfallIntensity)
          : null,
        waterSpreadPercentage: Number.isFinite(zone.waterSpread)
          ? Number(zone.waterSpread)
          : null,
      },
      update: {
        incidentId,
  ...
  const plannedAllocations =
    ((state.coordinatorOutput as any)?.resource_allocation || []) as any[];

  for (const [index, allocation] of plannedAllocations.entries()) {
    const resource = (state.resources as any[]).find(
      (item) =>
        item.id === allocation.resource ||
        item.type === allocation.resource,
    );

    if (!resource) continue;

    const zoneId = (state.zones as any[]).some(
      (zone) => zone.id === allocation.zone,
    )
      ? allocation.zone
      : null;
    ...
    await prisma.resourceAllocation.upsert({
      ...
      create: {
        ...
        allocatedQuantity:
          resource.total === 1
            ? 1
            : resource.id === 'MED-KITS'
              ? 5
              : 25,
        reason:
          allocation.reason ||
          allocation.priority ||
          'Coordinator response plan allocation',
      },
      update: {
        resourceId: resource.id,
        zoneId,
  ...
  for (const message of (state.messages || []) as any[]) {
    const agent =
      agentDefinitions.find(
        (item) => item.name === message.from_agent,
      ) || agentDefinitions[3];
  ...
  for (const [index, conflict] of (
    (state.conflicts || []) as any[]
  ).entries()) {
    const id =
      conflict.id || `CONFLICT-${incidentId}-${index}`;

    const resourceId = (state.resources as any[]).some(
      (resource) => resource.id === conflict.resource,
    )
      ? conflict.resource
      : null;
  ...
  if (coordinatorOutput) {
    await prisma.responsePlan.upsert({
      ...
      create: {
        ...
        evidence: JSON.stringify(
          coordinatorOutput.evidence || [],
        ),
        constraint: JSON.stringify(
          coordinatorOutput.constraints || [],
        ),
        tradeoff: JSON.stringify(
          coordinatorOutput.tradeoffs || [],
        ),
    ...
    if (coordinatorOutput.public_alert) {
      await prisma.publicAlert.upsert({
        where: { id: `ALERT-${incidentId}` },
        create: {
          id: `ALERT-${incidentId}`,
          incidentId,
          alertType: 'COORDINATED_RESPONSE',
          message: coordinatorOutput.public_alert,
          severity,
          createdAt: now,
        },
        update: {
  ...
  for (const entry of (state.history || []) as any[]) {
    await prisma.simulationHistory.upsert({
      where: { id: entry.id },
      create: {
        id: entry.id,
        incidentId,
  ...
  for (const observation of monitoring?.observations || []) {
    ...
    await prisma.satelliteObservation.upsert({
      ...
      create: {
        ...
        zoneId,
        riskLevel:
          observation.floodStatus ||
          observation.riskLevel ||
          'UNKNOWN',
        waterSpreadPercentage: Number.isFinite(
          observation.waterSpread,
        )
          ? Number(observation.waterSpread)
          : null,
        waterLevel: observation.waterLevel
          ? String(observation.waterLevel)
          : null,
        rainfallIntensity: Number.isFinite(
          observation.rainfallIntensity,
        )
          ? Number(observation.rainfallIntensity)
          : null,
        peopleAtRisk:
          Number(observation.peopleAtRisk) || 0,
        dataSource:
          monitoring.source || 'simulation',
        observedAt,
      },
      update: {
        riskLevel:
